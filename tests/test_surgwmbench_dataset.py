...
from planet_torch_surgwmbench.data.surgwmbench import SOURCE_TO_CODE, SurgWMBenchClipDataset, SurgWMBenchFrameDataset
...
def test_raw_video_dataset_frame_fallback_windows(tmp_path):
    root = make_surgwmbench_root(tmp_path)
    ...
    assert sample["frame_indices"].tolist() == list(range(8))


def test_raw_video_dataset_opencv_missing_error(tmp_path, monkeypatch):
    root = make_surgwmbench_root(tmp_path)
