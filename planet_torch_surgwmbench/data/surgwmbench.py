...
The loader follows the SurgWMBench manifest contract and does not create or
infer train/val/test splits. Sparse human anchors remain the primary target;
...
from planet_torch_surgwmbench.data.transforms import load_rgb_frame
...
def resolve_dataset_path(dataset_root: str | Path, value: str | Path | None) -> Path | None:
    """Resolve a dataset-relative path while preserving the stored value."""
...
class SurgWMBenchFrameDataset(Dataset):
    """Frame-level dataset for optional encoder/decoder pretraining."""
    ...
    def __init__(
        ...
        for clip_idx, entry in enumerate(self.clip_dataset.entries):
            ...
            num_frames = int(entry.get("num_frames", annotation.get("num_frames", len(frame_records))))
            paths = self.clip_dataset._paths_for_indices(entry, frame_records, list(range(num_frames)))
