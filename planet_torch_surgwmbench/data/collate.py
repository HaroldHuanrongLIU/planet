...
from planet_torch_surgwmbench.data.surgwmbench import NUM_HUMAN_ANCHORS, _metadata_from_sample
...
def collate_sparse_anchors(batch: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if frames.shape[1] != NUM_HUMAN_ANCHORS:
        raise ValueError(f"Sparse SurgWMBench batches must have {NUM_HUMAN_ANCHORS} frames, got {frames.shape[1]}")
    if coords_norm.shape[1] != NUM_HUMAN_ANCHORS or sampled_indices.shape[1] != NUM_HUMAN_ANCHORS:
        raise ValueError(f"Sparse SurgWMBench batches must have {NUM_HUMAN_ANCHORS} coordinates and sampled indices")
...
def collate_frame_autoencoding(batch: list[tuple[torch.Tensor, dict[str, Any]]]) -> dict[str, Any]:
    """Collate frame-level autoencoding samples."""
...
def collate_video_windows(batch: list[dict[str, Any]]) -> dict[str, Any]:
    """Collate raw-video or extracted-frame windows into a padded batch."""

    if not batch:
        raise ValueError("Cannot collate an empty batch")
    if any(item["frames"] is None for item in batch):
        raise ValueError("collate_video_windows requires samples with frames")
...
collate_frame_vae = collate_frame_autoencoding
